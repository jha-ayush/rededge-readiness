...
**Live:** <a href="https://rededge-readiness.sudokodes.workers.dev/" target="_blank" rel="noopener noreferrer">rededge-readiness.sudokodes.workers.dev</a>
...
| `test_rededge.py` | Stdlib unittest suite: shared readiness logic, the check contract, robustness against malformed and wrong-typed payloads, settings sanitizing, the local serve proxy, its headers and the page's sibling assets, and the offload walk. It also parses `web/_headers` and holds the hosted policy to the page, and reads the page's markup to hold the serve allowlist to every asset the page names. Run with `python3 -m unittest test_rededge`. Runs in CI. |
...
| `american_english_check.py` | Holds the tree to American English: every text file is read against an explicit list of British spellings and the build fails on one. Run with `python3 american_english_check.py`. Runs in CI. |
...
| `web/apple-touch-icon.png` | The Home Screen icon (180 by 180) for a phone or tablet that adds the page to its Home Screen, served beside the page. Without it, iOS uses a screenshot of the page. |
...
| `.github/workflows/` | CI on every push and pull request: Python compile, test suite, the American English guard, syntax check of both JavaScript clients, the cross-client parity harness, and the web config boundary guard. |
