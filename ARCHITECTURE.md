...
- **An American English guard** (`american_english_check.py`), reading every text file against an explicit list of British spellings, because the three clients and their documents read as one voice or they do not.
- **CI on every push and pull request**, which compiles the Python, runs the suite and the spelling guard, syntax-checks both JavaScript clients, and runs the parity harness and the configuration guard. Two of the three clients are JavaScript, so a Python-only pipeline could not see them.
